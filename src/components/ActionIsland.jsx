...
export default function ActionIsland({ data, busy, onChallenge, onDay, onNavigate }) {
  ...
  return <section className="island-experience" aria-label="Pulau Aksi Penyu interaktif">
    <div className="island-scene">
      ...
      <div className="island-location"><span>SEATLE • MISI 04</span><h2>Pulau Aksi Penyu</h2><p>{count === 7 ? 'Lihat, pantainya kembali bersih!' : 'Satu aksi darimu, satu harapan untuk laut.'}</p></div>
      <button className="island-building island-hut" onClick={() => onNavigate('campaign')}><Hut /><span>Pondok kreativitas<ArrowRight size={13} /></span></button>
      <button className="island-building island-board" onClick={() => onNavigate('gallery')}><Hut gallery /><span>Papan kampanye<ArrowRight size={13} /></span></button>
