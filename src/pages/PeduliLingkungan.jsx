...
const button = 'inline-flex min-h-12 items-center justify-center gap-2 rounded-full border-2 border-white bg-teal-700 px-6 py-3 text-sm font-black text-white shadow-md transition hover:bg-teal-800 focus-visible:outline focus-visible:outline-4 focus-visible:outline-sky-600 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500';
...
export default function PeduliLingkungan({ onBack }) {
  ...
  return (
    <main className="care-ocean page-background relative min-h-screen overflow-x-hidden bg-sky-800 pb-16 text-sky-950" style={{ '--page-background': `url(${background})` }}>
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-sky-950/65 via-teal-950/40 to-sky-950/60" />
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-3 sm:px-6">
        ...
        <div className="care-heading">
          ...
          <div className="care-completion" role="progressbar" aria-label="Bagian refleksi tersimpan" aria-valuemin={0} aria-valuemax={3} aria-valuenow={completedCount}><strong>{completedCount}<span>/3</span></strong><small>bagian tersimpan</small></div>
        </div>
        ...
        {data.finished ? (
          <section className={`${card} px-7 py-12 sm:px-14`}>
            <div className="text-center">
              ...
              <p className="mt-4 text-xs font-black uppercase tracking-widest text-teal-700">Tiga langkah kepedulian selesai</p>
            ...
            <div className="mx-auto mt-7 max-w-2xl rounded-3xl border-2 border-teal-200 bg-gradient-to-br from-teal-50 via-white to-amber-50 p-6 sm:p-8">
              <div className="flex items-center justify-between text-teal-700"><span className="text-xs font-black uppercase tracking-widest">Janjiku untuk laut</span><Waves className="h-6 w-6" /></div>
              <p className="mt-4 break-words font-brand text-xl leading-relaxed">Mulai hari ini saya berkomitmen untuk {data.action.trim()}<br /><span className="text-teal-700">Agar {data.purpose.trim()}</span></p>
              <div className="mt-6 border-t border-teal-200 pt-5"><p className="font-black"><span aria-hidden="true" className="mr-2 text-2xl">{selectedFeeling?.emoji}</span>{selectedFeeling?.label}</p><p className="mt-2 break-words text-sm leading-relaxed">{data.reason}</p></div>
              <details className="mt-5 border-t border-teal-200 pt-4"><summary className="cursor-pointer text-sm font-black text-sky-800">Lihat 5 jawaban refleksi sikapku</summary><ol className="mt-4 space-y-4">{statements.map((statement, index) => <li key={statement} className="text-sm leading-relaxed"><p>{index + 1}. {statement}</p><p className="mt-1 font-black text-teal-700">{data.answers[index]}</p></li>)}</ol></details>
            </div>
          ...
          <div className="care-activity">
            <section className={`${card} care-activity-panel`}>
              ...
              <div ref={heading} tabIndex={-1} className="scroll-mt-6 focus:outline-none">
                <p className="text-xs font-black uppercase tracking-widest text-teal-700">0{step + 1} / {steps[step].subtitle}</p>
                <h2 className="mt-2 font-brand text-3xl font-semibold sm:text-4xl">{steps[step].title}</h2>
              </div>
              <form onSubmit={confirmStep} className="mt-5">
                {step === 0 && (
                  <>
                    ...
                    <div className="care-commitment-grid">
                      ...
                      <aside className="care-promise-preview">
                        <div className="mb-4 flex items-center gap-2 text-xs font-black uppercase tracking-widest text-teal-700"><Waves className="h-4 w-4" /> Komitmenmu</div>
                        <p className="break-words font-brand text-xl leading-relaxed sm:text-2xl">Mulai hari ini saya berkomitmen untuk <span className={data.action.trim() ? 'text-teal-700' : 'text-sky-400'}>{data.action.trim() || '…'}</span></p>
                        <p className="mt-4 break-words font-brand text-xl leading-relaxed sm:text-2xl">Agar <span className={data.purpose.trim() ? 'text-teal-700' : 'text-sky-400'}>{data.purpose.trim() || '…'}</span></p>
                        <div className="mt-5 flex items-center justify-between gap-3 border-t border-teal-200 pt-4"><span className="min-w-0 break-words text-sm font-black text-teal-800">Dari {studentName}, untuk laut</span><Waves className="h-7 w-7 shrink-0 text-teal-500" /></div>
                      </aside>
                    </div>
                  </>
                )}
                  ...
                  <>
                    ...
                    <fieldset className="mt-5"><legend className="text-sm font-bold text-sky-700">Pilih satu emoji yang paling menggambarkan perasaanmu.</legend><div className="care-feelings">{feelings.map((feeling) => (
                      <label key={feeling.id} className="care-feeling relative cursor-pointer">
                        ...
                        <span className="flex h-full min-h-32 flex-col items-center justify-center gap-3 rounded-3xl border-2 border-sky-100 bg-white p-3 text-center transition hover:border-teal-300 peer-checked:border-teal-600 peer-checked:bg-teal-50 peer-checked:shadow-md peer-focus-visible:ring-4 peer-focus-visible:ring-sky-400"><span aria-hidden="true" className="text-4xl">{feeling.emoji}</span><span className="text-sm font-black">{feeling.label}</span>{data.feeling === feeling.id && <Check aria-hidden="true" className="absolute right-3 top-3 h-4 w-4 text-teal-700" />}</span>
                      </label>
                    ))}</div></fieldset>
                    <div className="my-5 flex items-center gap-3 rounded-2xl bg-teal-50 p-4 text-sm font-bold text-teal-900" aria-live="polite"><Heart className="h-5 w-5 shrink-0" />{selectedFeeling ? `Kamu memilih “${selectedFeeling.label}”. Yuk, ceritakan alasannya.` : 'Setiap perasaan berharga. Tidak ada jawaban benar atau salah.'}</div>
                ...
                {step === 2 && (
                  <>
                    ...
                    <fieldset key={statementIndex} className="care-statement">
                      <legend>{statements[statementIndex]}</legend>
                ...
                {data.confirmed[step] && <div role="status" className="mt-6 flex items-start gap-3 rounded-2xl border border-teal-200 bg-teal-50 p-5 text-sm font-bold leading-relaxed text-teal-900"><Check className="mt-0.5 h-5 w-5 shrink-0" /><p>{feedback[step]}</p></div>}
